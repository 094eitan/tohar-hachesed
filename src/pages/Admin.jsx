...
export default function Admin() {
  ...
  const addDelivery = async () => {
    ...
    // אם הוזנה שכונה ידנית—להוסיף/להפעיל ברשימת השכונות
    if (neighborhood) {
  ...
  /** ───────── ייבוא קובץ (CSV/XLSX/XLS) עם דוח שגיאות + הכנסת שכונות ───────── */
    ...
    try{
      ...
      if (ext === 'csv'){
        ...
        rawObjects = dataRows.map((r,idx)=>{
          ...
          o.__rowIndex = headerRowIdx + 2 + idx // מספר שורה אנושי (1-based)
          return o
        })
      } else {
      ...
      // הוספת שכונות שנמצאו באקסל לרשימה (מופעלות)
  ...
  const deleteAll = async (scope='filtered') => {
    const target = scope==='all' ? rows : visible
    if (!target.length) { alert('אין מה למחוק'); return }
    if (!confirm(`למחוק ${target.length} רשומות? (אי אפשר להחזיר)`)) return
    for (const r of target) {
      await deleteDoc(doc(db, 'deliveries', r.id))
    }
  }
  ...
  const addNeighborhood = async () => {
    const name = prompt('שם שכונה:'); if (!name) return
    ...
    <Wrap>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        ...
        <div className="dropdown">
          <div tabIndex={0} role="button" className="btn btn-error btn-outline">מחיקה…</div>
          <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-56">
            <li><button onClick={()=>deleteAll('filtered')}>מחק את המסוננים</button></li>
            <li><button onClick={()=>deleteAll('all')}>מחק את כולם</button></li>
          </ul>
        </div>
      </div>
      ...
      <div className="overflow-x-auto mt-3">
        <table className="table table-zebra w-full">
          ...
          <tbody>
            {visible.map(r => (
              <tr key={r.id}>
                ...
                <td className="whitespace-nowrap">
                  <EditableCell
                    value={r.phone || ''}
                    onSave={val => updateDoc(doc(db,'deliveries',r.id), { phone: val, updatedAt: serverTimestamp() })}
                  />
                </td>
                <td className="whitespace-nowrap max-w-[280px]">
                  <EditableCell
                    value={r.notes || ''}
                    textarea
                    onSave={val => updateDoc(doc(db,'deliveries',r.id), { notes: val, updatedAt: serverTimestamp() })}
                  />
                </td>
                ...
                <td className="flex flex-wrap gap-1">
                  <div className="join">
                    <button className="btn btn-xs join-item" onClick={() => updateStatus(r.id, 'pending')}>ממתין</button>
            ...
            {visible.length === 0 && (
              <tr><td colSpan="8" className="opacity-60">אין נתונים לתצוגה</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
async function updateAddressField(id, key, value){
  ...
  // אם ערכו שכונה חדשה – להוסיף/להפעיל גם ב-neighborhoods
