...
export default function AdminVolunteers() {
  ...
  // --- Auth state (לא מחזירים לפני שכל ה-Hooks הוגדרו) ---
  ...
  useEffect(()=>{
    ...
    const un = onSnapshot(
      ...
      err=> setErrMsg('אין הרשאה/שגיאה בקריאת volunteers: '+(err?.message||err))
    )
    return ()=>un()
  }, [authChecked, allowed])

  // "מחובר עכשיו" (90 שנ׳ אחרונות)
  function isOnline(v){
  ...
  return (
    <Wrap>
      ...
      {/* החלקים הבאים מוצגים רק כשיש הרשאת אדמין */}
      {authChecked && allowed && (
        <>
              ...
              <div className="overflow-x-auto">
                <table className="table table-zebra w-full">
                  <thead className="sticky top-0 bg-base-100">
                    <tr>
                      <th>#</th><th>שם</th><th>אימייל</th><th>סטטוס</th><th>שובצו</th><th>נמסרו</th><th>נראה לאחרונה</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((v,idx)=>{
